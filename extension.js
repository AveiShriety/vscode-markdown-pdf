...
function activate(context) {
  ...
  var commands = [
    //--start--avei 2023-08-21: add convert all md file in project to target format files
    vscode.commands.registerCommand('extension.markdown-pdf.allMD_html', async function () { 
      allProjectMDFiles = [];
      await allMD_markdownPdf('html'); 
    }),
    vscode.commands.registerCommand('extension.markdown-pdf.allMD_pdf', async function () { 
      allProjectMDFiles = [];
      await allMD_markdownPdf('pdf'); 
    }),
    //--end--avei 2023-08-21: add convert all md file in project to target format files
...
//--start--avei 2023-08-21: add convert all md file in project to target format files
var allProjectMDFiles = [];
function walkSync(currentDirPath) {
  fs.readdirSync(currentDirPath, { withFileTypes: true }).forEach(function (dirent) {
    var filePath = path.join(currentDirPath, dirent.name);
    if (dirent.isFile() && path.extname(filePath) == '.md') {
      allProjectMDFiles.push(filePath);
  ...
  return allProjectMDFiles;
}
async function allMD_markdownPdf(option_type) {
  let rootFolders = vscode.workspace.workspaceFolders;
  // let rootFolders = ['/Users/feng/Desktop/SELF_CODES/vscode-markdown-pdf'];
  for (let i = 0; i < rootFolders.length; i++) {
    //folder.name
    let allRootFolderMDFiles = walkSync(rootFolders[i].uri.fsPath);
    vscode.window.showInformationMessage(`${rootFolders[i].uri.fsPath} has md-files count[${allRootFolderMDFiles.length}]`)
    // let allRootFolderMDFiles = walkSync(rootFolders[i]);
    for (let j = 0; j < allRootFolderMDFiles.length; j++) {
      var textDocument = await vscode.workspace.openTextDocument(allRootFolderMDFiles[j]);
